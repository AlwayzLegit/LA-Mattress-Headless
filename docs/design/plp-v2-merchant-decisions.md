...
| ☐ | **A4** | **Long-content source-of-truth (UPDATED 2026-05-20 per Phase 2.5 audit CSV — see audit §0).** Earlier framing offered two options; the live data made it one: `custom.seo_content` is populated on **49 of 64 collections** (~586KB), so we don't delete it — we wire it. **Phase A (this RFC, code in flight):** render `collection.descriptionHtml` below grid (same content as today, just relocated). **Phase B (follow-up RFC):** add a rich-text-JSON → HTML serializer, wire `custom.seo_content` as the primary below-grid source. Approve this two-phase split? | Phase A now, Phase B follow-up | |
...
| ☐ | **B2** | **Approve REVISED Phase 3 deletions** (audit §0, CSV at `data/seo-metafields/data-audit-2026-05-20.csv`). The 14 originally listed are now **3 confirmed-orphan groups** safe to delete: (a) 10 link/label fields `link1`–`link5` + `label1`–`label5` (zero data across all 64 collections); (b) `custom.sleep_position` PRODUCT singular (4 floor-sample products with light values, plural `sleep_positions` is canonical). Holds on: `custom.description_` (25 collections have ~26KB of content — merchant decides: merge into seo_content vs keep + wire), `custom.link`+`custom.label` (1 collection populated — confirm purpose), `custom.seo_content` (49 collections, ~586KB — KEEP, Phase B wires it). Default "delete definition only" — data on resources persists if recovery is ever needed. | yes for the 11 truly-empty + the 4 floor-sample sleep_position rows | |
