...
Storefront contact: phone numbers and addresses for each showroom live on the /pages/mattress-store-locations page. The general line is ${SITE_PHONE_DISPLAY} — it rings the central order desk and routes to whichever showroom can help fastest. Each showroom also has a direct local number on the locations page if the shopper wants to reach a specific store.
