...
export async function searchProducts(query: string, opts: { first?: number; after?: string | null } = {}): Promise<SearchResult> {
  const data = await shopifyFetch<Raw>(SEARCH_PRODUCTS, { query, first: opts.first ?? 24, after: opts.after ?? null });
  return {
