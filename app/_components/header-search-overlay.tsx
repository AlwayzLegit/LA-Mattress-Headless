 ...
 *
 * Phase 183 split: the trigger button (always rendered into the nav)
 * lives in header-search.tsx; this file holds the overlay implementation
 * and is loaded lazily via `next/dynamic` only when the user opens the
 * search — predictive-search code, focus trap, scroll lock, and portal
 * machinery all defer to first interaction.
 *
 * Structure (unchanged from the pre-split version):
 ...
 *       .search-foot       keyboard hints
 *
 * Body scroll is locked while the overlay is open. The component is
 * `ssr: false` via the dynamic import, so `document` is always defined
 * when this code runs.
 */
export function HeaderSearchOverlay({ onClose }: { onClose: () => void }) {
  ...
  // Hydrate recent searches once on mount (i.e. when the overlay first
  // opens). Reading localStorage was previously gated behind an `open`
  // flag — now the mount lifecycle achieves the same deferral.
  useEffect(() => {
